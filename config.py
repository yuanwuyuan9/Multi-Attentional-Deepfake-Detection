...
class train_config:
    def __init__(self,name,recipes=[],**params):
        ...
        self.train_dataset=dict(datalabel=self.datalabel, resize=self.resize,normalize=self.normalize,augment=self.augment)
        self.val_dataset=self.train_dataset
        self.net_config=dict(net=self.net,feature_layer=self.feature_layer,attention_layer=self.attention_layer,num_classes=self.num_classes, M=self.num_attentions,mid_dims=self.mid_dims,dropout_rate=self.dropout_rate,drop_final_rate=self.drop_final_rate,pretrained=self.pretrained,alpha=self.alpha,size=self.resize,margin=self.margin,inner_margin=self.inner_margin)
